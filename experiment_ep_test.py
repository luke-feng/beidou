...
from local_node_fedep import local_node_fedep
import pickle
...
# maxRound = 30
# maxEpoch = 1
# init_aggregation_dict = {'fed_avg':fed_avg, 'krum':krum, 'trimmedMean':trimmedMean, 'median':median, 'fedep', fedep}
...
# 6月22日 fedavg 
# num_peers = 10
# alpha_list = [20, 5, 1, 0.5, 0.1]
# topology_list = ["fully"]
# maxRound = 30
# maxEpoch = 10
# init_aggregation_dict = {'fedep':fedep}
        ...
        for init_aggregation_name in init_aggregation_dict:
            ...
            for attack_type in attack_type_list:
                for poisoned_node_ratio in poisoned_node_ratio_list:
                    ...
                    experimentsName = f'TrialRun_{num_peers}_clients_alpha_{alpha}_{dataset_name}_{topology}_{init_aggregation_name}_{attack_type}_{poisoned_node_ratio}_dynamic_topo_{dynamic_topo}_dynamic_agg_{dynamic_agg}_dynamic_data_{dynamic_data}_is_proactive_{is_proactive}'+dt_string
                    ...
                    for client in range(num_peers):
                        ...
                        if init_aggregation_name=='fedep':
                            node = local_node_fedep(node_id,basic_config, data_train_loader, data_val_loader, test_dataset_loader, backdoor_valid_loader)
                        else:
                    ...
                    if init_aggregation_name=='fedep':
                        for node_id in node_list:
                            ...
                            node.fit_distribution()
                            for nei in node_list:
                                node_list[nei].add_distribution(node_id, node.get_theta(), node.get_data_size())
                                
                        for node_id in node_list:
                            node = node_list[node_id]
                            global_unique_targets = np.unique(train_dataset.targets)
                            node.pooling(global_unique_targets)
...
experiment_elapsed_time_formatted = str(timedelta(seconds=experiment_elapsed_time))
    
print(f"Experiment took {experiment_elapsed_time_formatted}.")
